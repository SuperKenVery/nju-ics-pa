...
#define assert_res_CPSZO(dataSize) \
	...
	fflush(stdout); \
  if(res!=res_asm || cpu.eflags.OF!=test_eflags.OF) printf("res=%p, res_asm=%p, a=%p, b=%p, real of=%d, emu of=%d\n",(void*)res,(void*)res_asm,(void*)a,(void*)b,test_eflags.OF,cpu.eflags.OF); \
	assert(res == res_asm); \
